...
@section('title', 'Master Superadmin Console')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-slate-950 via-purple-950 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border border-purple-900/40">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('superadmin.users') }}" class="px-5 py-2.5 bg-purple-500 hover:bg-purple-400 text-slate-950 font-black text-xs rounded-xl shadow transition flex items-center gap-1.5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2">
            ...
            <div class="text-2xl font-black text-slate-900">{{ $totalUsers }} Akun</div>
            <div class="text-[10px] text-purple-700 font-bold">{{ $totalMitras }} Mitra • {{ $totalAdmins }} Admin</div>
        </div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2">
            ...
            <div class="text-xl font-black text-emerald-700">Rp {{ number_format($totalGmv, 0, ',', '.') }}</div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2">
            ...
            <div class="text-xl font-black text-slate-900">Rp {{ number_format($platformFeeTotal, 0, ',', '.') }}</div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2">
            ...
            <div class="text-xl font-black text-blue-700">Rp {{ number_format($totalEscrow, 0, ',', '.') }}</div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-4">
        <div class="flex items-center justify-between pb-3 border-b border-slate-100">
            ...
            <a href="{{ route('superadmin.payouts') }}" class="text-xs font-bold text-purple-700 hover:underline">Kelola Semua Payout &rarr;</a>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 font-medium">
                    @forelse($pendingPayouts as $p)
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="p-3">
                                <div class="font-bold text-slate-900">{{ $p->mitra->name }}</div>
                                <div class="text-[10px] text-slate-400">{{ $p->mitra->email }}</div>
                            </td>
                            ...
                            <td class="p-3 text-right space-x-1">
                                <form action="{{ route('superadmin.payouts.approve', $p->id) }}" method="POST" class="inline">
                                    @csrf
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-4">
        <div class="flex items-center justify-between pb-3 border-b border-slate-100">
            ...
            <a href="{{ route('superadmin.audit_logs') }}" class="text-xs font-bold text-purple-700 hover:underline">Buka Semua Log &rarr;</a>
        </div>
                ...
                <div class="p-3 bg-slate-50 rounded-2xl border border-slate-200/80 flex items-center justify-between text-xs">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span class="text-slate-400">pada {{ $log->entity_type }} (ID: {{ $log->entity_id }})</span>
                        </div>
                    </div>

                    <div class="text-right text-[11px] text-slate-400">
                        <span class="font-mono">{{ $log->ip_address }}</span> • {{ $log->created_at->diffForHumans() }}
                    </div>
                </div>
            @endforeach
        </div>
