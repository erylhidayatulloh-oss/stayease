...
@section('title', 'Otorisasi Pencairan Dana Mitra')
                ...
                <tbody class="divide-y divide-slate-100 font-medium">
                    @forelse($payouts as $p)
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="p-4 text-slate-700 font-mono">{{ $p->created_at->format('d M Y, H:i') }}</td>
                            <td class="p-4">
                                <div class="font-bold text-slate-900">{{ $p->mitra->name }}</div>
                                <div class="text-[10px] text-slate-400 font-mono">{{ $p->mitra->email }}</div>
                            </td>
                            ...
                            <td class="p-4 text-right space-x-1">
                                @if($p->status === 'pending')
                                    ...
                                    <form action="{{ route('superadmin.payouts.reject', $p->id) }}" method="POST" class="inline" onsubmit="const r = prompt('Masukkan alasan penolakan pencairan dana:'); if(r){ this.notes.value = r; return true; } return false;">
                                        @csrf
