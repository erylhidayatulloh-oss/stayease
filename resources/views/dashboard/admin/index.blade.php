...
@section('title', 'Admin Operasional & Verifikasi')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-950 rounded-3xl p-6 sm:p-8 text-white shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.verification_queue') }}" class="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs rounded-xl shadow transition flex items-center gap-1.5">
                <span>🛡️ Antrean Verifikasi ({{ $pendingPropertiesCount }})</span>
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2">
            ...
            <div class="text-2xl font-black text-amber-600">{{ $pendingPropertiesCount }} Listing</div>
            <div class="text-[10px] text-slate-500 font-medium"><a href="{{ route('admin.verification_queue') }}" class="text-emerald-700 hover:underline">Buka Antrean Audit &rarr;</a></div>
        </div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2">
            ...
            <div class="text-2xl font-black text-slate-900">{{ $activePropertiesCount }} Properti</div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2">
            ...
            <div class="text-2xl font-black text-slate-900">{{ $totalBookingsCount }} Pesanan</div>
            <div class="text-[10px] text-slate-500 font-medium"><a href="{{ route('admin.bookings') }}" class="text-emerald-700 hover:underline">Monitor Pembayaran &rarr;</a></div>
        </div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2">
            ...
            <div class="text-xl font-black text-slate-900">Rp {{ number_format($totalGmv, 0, ',', '.') }}</div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-4">
        <div class="flex items-center justify-between pb-3 border-b border-slate-100">
            ...
            <a href="{{ route('admin.verification_queue') }}" class="text-xs font-bold text-emerald-700 hover:underline">Lihat Semua ({{ $pendingPropertiesCount }}) &rarr;</a>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 font-medium">
                    @forelse($pendingQueue as $p)
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <td class="p-3">
                                <div>{{ $p->mitra->name }}</div>
                                <div class="text-[10px] text-slate-400">{{ $p->mitra->phone }}</div>
                            </td>
                            <td class="p-3">{{ strtoupper($p->property_type) }} • {{ $p->sub_district }}, {{ $p->city->name ?? '' }}</td>
                            <td class="p-3 font-black text-slate-900">Rp {{ number_format($p->base_price_monthly, 0, ',', '.') }}/bln</td>
                            <td class="p-3 text-right space-x-1">
                                <form action="{{ route('admin.properties.approve', $p->id) }}" method="POST" class="inline">
                                    @csrf
