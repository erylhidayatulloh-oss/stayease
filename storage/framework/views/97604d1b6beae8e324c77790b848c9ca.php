<?php $__env->startSection('title', 'Beranda Penyewa'); ?>
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-emerald-900 via-slate-900 to-teal-950 rounded-3xl p-6 sm:p-8 text-white shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div class="space-y-2">
            ...
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Selamat Datang, <?php echo e($user->name); ?>! 👋</h1>
        ...
        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('user.bookings')); ?>" class="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs rounded-xl shadow transition">
    ...
    <?php if($activeBookings->count() > 0): ?>
        <?php $active = $activeBookings->first(); ?>
        <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                ...
                <span class="px-3 py-1 bg-emerald-100 text-emerald-800 text-xs font-black rounded-full">
                    STATUS: <?php echo e(strtoupper(str_replace('_', ' ', $active->status))); ?>

                </span>
            </div>

            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                <img src="<?php echo e($active->property->images[0] ?? 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=600&q=80'); ?>" 
                     alt="<?php echo e($active->property->title); ?>" 
                ...
                <div class="space-y-2 flex-1 min-w-0">
                    <span class="text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md">
                        Kode Booking: <?php echo e($active->booking_code); ?>

                    </span>
                    <h4 class="font-extrabold text-base text-slate-900 truncate"><?php echo e($active->property->title); ?></h4>
                    <p class="text-xs text-slate-500"><?php echo e($active->unit->name); ?> • <?php echo e($active->property->sub_district); ?>, <?php echo e($active->property->city->name ?? 'Indonesia'); ?></p>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1 text-xs">
                        <div class="bg-slate-50 p-2 rounded-xl">
                            ...
                            <strong class="text-slate-800"><?php echo e($active->check_in_date->format('d M Y')); ?></strong>
                        </div>
                        <div class="bg-slate-50 p-2 rounded-xl">
                            <span class="text-[10px] text-slate-400 block">Durasi Sewa</span>
                            <strong class="text-slate-800"><?php echo e($active->duration_months); ?> Bulan</strong>
                        </div>
                        <div class="bg-slate-50 p-2 rounded-xl col-span-2 sm:col-span-1">
                            <span class="text-[10px] text-slate-400 block">Total Biaya</span>
                            <strong class="text-emerald-700 font-black">Rp <?php echo e(number_format($active->grand_total, 0, ',', '.')); ?></strong>
                        </div>
                    </div>
                </div>

                <div class="flex flex-col gap-2 w-full sm:w-auto shrink-0">
                    <a href="<?php echo e(route('user.contract', $active->id)); ?>" class="w-full sm:w-auto px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl text-center shadow-sm flex items-center justify-center gap-1.5 transition">
                        <span>📜 Surat Perjanjian Sewa</span>
                    </a>
                    <a href="<?php echo e(route('user.messages')); ?>" class="w-full sm:w-auto px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl text-center flex items-center justify-center gap-1.5 transition">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-2xl font-black text-slate-900"><?php echo e($activeBookings->count() + $completedBookingsCount); ?></div>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-2xl font-black text-slate-900"><?php echo e($wishlists->count()); ?> Properti</div>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-2xl font-black text-slate-900"><?php echo e($unreadMessagesCount); ?> Belum Dibaca</div>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            ...
            <a href="<?php echo e(route('user.wishlist')); ?>" class="text-xs font-bold text-emerald-700 hover:underline">Lihat Semua &rarr;</a>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <?php $__empty_1 = true; $__currentLoopData = $wishlists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $w): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-md transition flex flex-col justify-between">
                    <div>
                        <img src="<?php echo e($w->property->images[0] ?? ''); ?>" alt="<?php echo e($w->property->title); ?>" class="w-full h-32 object-cover">
                        <div class="p-3 space-y-1">
                            <span class="text-[10px] font-bold px-1.5 py-0.5 bg-slate-100 rounded text-slate-700"><?php echo e(strtoupper($w->property->property_type)); ?></span>
                            <h4 class="font-bold text-xs text-slate-900 truncate"><?php echo e($w->property->title); ?></h4>
                            <p class="text-[11px] text-slate-500"><?php echo e($w->property->sub_district); ?></p>
                            <div class="text-xs font-black text-emerald-700">Rp <?php echo e(number_format($w->property->base_price_monthly, 0, ',', '.')); ?>/bln</div>
                        </div>
                    </div>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/macbookair/SevenInc/StayEase-Revisi/resources/views/dashboard/user/index.blade.php ENDPATH**/ ?>